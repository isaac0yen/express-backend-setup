...
export const logger = {
  ...
  format: null as FormatFunction | null,
  ...
  log(level: string, message: string, content?: unknown) {
    ...
    const lineNumber = stackLine?.split(":")?.[1] || 'unknown';
    const filePath = stackLine?.split("(")?.[1]?.split(":")?.[0] || 'unknown';

    const formatFunction = this.format || this.defaultFormat.bind(this);
    const logEntry = formatFunction(
      level,
      message,
      logContent,
      timestamp,
